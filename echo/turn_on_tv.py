...
class device_handler(debounce_handler):
    ...
    """
    TRIGGERS = {"the tv": 52001}

    def act(self, client_address, state):
        print "State", state, "from client @", client_address
        # Are we sending it an "on" command?
        if state:
            # Is the TV currently off? If so, turn it on
            if "power status: on" not in subprocess.check_output("echo pow 0 | cec-client -s -d 1", shell=True):
                subprocess.call("echo on 0 | cec-client -s -d 1", shell=True)
        # Are we sending it an "off" command?
        else:
            # Is the TV currently on? If so, turn it off
            if "power status: standby" not in subprocess.check_output("echo pow 0 | cec-client -s -d 1", shell=True):
